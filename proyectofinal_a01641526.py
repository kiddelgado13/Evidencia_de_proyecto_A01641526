...
etiqueta = cv2.imread("etiquetado2.webp")
...
rules = {
    ...
    "Low in Fat": {"GRASAS TOTALES": {"threshold": 3, "comparison": "<"}},
    "Low in Saturated Fat": {"GRASAS SATURADAS": {"threshold": 1.5, "comparison": "<"}},
    "Low in Sugar": {"AZUCARES": {"threshold": 5, "comparison": "<"}},
    "Low in Sodium": {"SODIO": {"threshold": 100, "comparison": "<"}} # per 100g
}
...
if is_unhealthy:
    food_classification = "Unhealthy"
else:
    is_healthy = True
    for rule, conditions in rules["Healthy"].items():
        if rule in nutrient_values_float and nutrient_values_float[rule] is not None:
             if conditions["comparison"] == ">" and nutrient_values_float[rule] <= conditions["threshold"]:
                 is_healthy = False
                 break
             if conditions["comparison"] == "<" and nutrient_values_float[rule] >= conditions["threshold"]:
                 is_healthy = False
                 break
        elif rule in rules["Healthy"] and nutrient_values_float.get(rule) is None:
             is_healthy = False # Cannot classify as healthy if required nutrient is missing
             break


    if is_healthy:
        food_classification = "Healthy"
    else:
        # Check for "Low in" classifications if not classified as Healthy or Unhealthy
        is_low_in_fat = False
        if "GRASAS TOTALES" in nutrient_values_float and nutrient_values_float["GRASAS TOTALES"] is not None:
            if nutrient_values_float["GRASAS TOTALES"] < rules["Low in Fat"]["GRASAS TOTALES"]["threshold"]:
                is_low_in_fat = True
        ...
        if "GRASAS SATURADAS" in nutrient_values_float and nutrient_values_float["GRASAS SATURADAS"] is not None:
             if nutrient_values_float["GRASAS SATURADAS"] < rules["Low in Saturated Fat"]["GRASAS SATURADAS"]["threshold"]:
        ...
        if "AZUCARES" in nutrient_values_float and nutrient_values_float["AZUCARES"] is not None:
            if nutrient_values_float["AZUCARES"] < rules["Low in Sugar"]["AZUCARES"]["threshold"]:
                is_low_in_sugar = True

        is_low_in_sodium = False
        if "SODIO" in nutrient_values_float and nutrient_values_float["SODIO"] is not None:
            if nutrient_values_float["SODIO"] < rules["Low in Sodium"]["SODIO"]["threshold"]:
...
print("Food Classification:", food_classification)
